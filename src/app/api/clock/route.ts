...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    console.log(`📍 ${type} requested by ${user!.id} (last was: ${lastType})`);

    if (type === 'CLOCK_IN') {
      if (lastType === 'CLOCK_IN' || lastType === 'BREAK_END') {
        await prisma.clockEvent.create({
          data: { userId: user!.id, type: 'CLOCK_OUT', note: 'Auto: switched device' },
        });
      } else if (lastType === 'BREAK_START') {
        await prisma.clockEvent.create({
          data: { userId: user!.id, type: 'BREAK_END', note: 'Auto: end break' },
        });
        await prisma.clockEvent.create({
          data: { userId: user!.id, type: 'CLOCK_OUT', note: 'Auto: switched device' },
        });
      ...
      if (lastType === 'BREAK_START') {
        await prisma.clockEvent.create({
          data: { userId: user!.id, type: 'BREAK_END', note: 'Auto: end break before clock-out' },
        });
      }
    }
      ...
      if (lastType === 'NONE' || lastType === 'CLOCK_OUT') {
        await prisma.clockEvent.create({
          data: { userId: user!.id, type: 'CLOCK_IN', note: 'Auto: clock-in before break' },
        });
      } else if (lastType === 'BREAK_START') {
        return NextResponse.json({
          event: null,
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const targetUserId = user!.role === 'ADMIN' && userIdParam ? userIdParam : user!.id;
    const from = fromParam ? new Date(fromParam) : new Date(Date.now() - 7 * 24 * 60 * 60 * 1000);
    const to = toParam ? new Date(toParam) : new Date();

    const events = await prisma.clockEvent.findMany({
      where: { userId: targetUserId, timestamp: { gte: from, lte: to } },
      orderBy: { timestamp: 'desc' },
    });
